...
async def set_afk(afk_e):
    ...
    if string:
        ...
        await afk_e.edit("🃏JESS-UBOT \n╭✠╼━━━━━━❖━━━━━━━✠╮ \n┣ __𝐋𝐚𝐠𝐢 𝐀𝐅𝐊__ \n╰✠╼━━━━━━❖━━━━━━━✠╯")
    if user.last_name:
        await afk_e.client(UpdateProfileRequest(first_name=user.first_name, last_name=user.last_name + " 🃏JESS-UBOT "))
    else:
        await afk_e.client(UpdateProfileRequest(first_name=user.first_name, last_name=" 🃏JESS-UBOT🃏 "))
    if BOTLOG:
...
async def type_afk_is_not_true(notafk):
    ...
    user = await bot.get_me()  # pylint:disable=E0602
    last = user.last_name
    if last and last.endswith(" 🃏JESS-UBOT🃏 "):
        last1 = last[:-12]
    else:
        last1 = ""
    ...
    if ISAFK:
        ISAFK = False
        msg = await notafk.respond("🃏JESS-UBOT ")
        time.sleep(3)
        await msg.delete()
    ...
    if mention.message.mentioned and not (await mention.get_sender()).bot:
        if ISAFK:
            ...
            if mention.sender_id not in USERS:
                if AFKREASON:
                    await mention.reply(f"🃏JESS-UBOT \n╭✠╼━━━━━━━━━━━━ \n┣ {ALIVE_NAME} ꜱᴇᴅᴀɴɢ ᴀꜰᴋ\n┣ ꜱᴇᴊᴀᴋ: {afk_since}\n┣ ᴀʟᴀꜱᴀɴ: {AFKREASON}\n╰✠╼━━━━━━━━━")
                else:
